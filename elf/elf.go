...
import (
	...
	"github.com/Gui774ume/gobpf/pkg/bpffs"
	"github.com/Gui774ume/gobpf/pkg/cpuonline"
)
...
func (b *Module) Load(parameters map[string]SectionParams) error {
	...
	for i, section := range b.file.Sections {
		...
		if section.Type == elf.SHT_REL {
			...
			if isKprobe || isKretprobe || isUprobe || isUretprobe || isCgroupSkb || isCgroupSock || isSockOps || isSocketFilter || isTracepoint || isSchedCls || isSchedAct || isXDP {
